 ...
 *
 * You can optionally connect a switch between pin 5 and ground
 ...
 *
 * You can also optionally connect a button between pin 4 and
...
const uint8_t dataPin = 6; //2 data pin for RGB LED strip
const uint8_t clockPin = 7; //3 clock pin for RGB LED strip

// Battery Voltage Monitor Setup
const float RB1=9.86; //battery monitor voltage divider resistance R1
const float RB2=2.17; //battery monitor voltage divider resistance R2
const float ArefVoltage=5.2; //supply voltage used as reference max voltage for Analog to Digital converter
const float MinBatteryVoltage=11.8; // Lights will be shut off when battery discharges to this voltage.
const float BatteryFullChargeVoltage=12.8;

const uint16_t BatteryMinVoltCount=1023.0*(MinBatteryVoltage*RB2/(RB2+RB1))/ArefVoltage;
bool batteryLowVoltageFault=false;

// Pattern Display Time Setup
const unsigned long PatternDisplayTime=3; //Time that pattern will display before turning off, in minutes
const unsigned long patternDisplayTime=PatternDisplayTime*60*1000; //conversion to milliseconds
static unsigned long patternDisplayStartTime; //will be set when displayPatterns is set to true
bool displayPatterns = false; //this variable will be set true after a long enough period of darkness, starting
                              //the light show, and will be set false again at the end of the patternDisplayTime

// Doorswitch Timer Setup
const unsigned long DoorOpenTimeout=1; //Lights will be all white while door is open until this time in minutes has elapsed, then the lights will turn off until door is closed and pattern display will resume.
const unsigned long doorOpenTimeout=DoorOpenTimeout*60*1000; //conversion to milliseconds

// Darkness Sensor Timer Setup
bool continuousDarkness = false; //state of darkness sensed by photocell, initialized to false (not dark yet)
const unsigned long DarknessTriggerTime=30;//=10 //length of time (minutes) darkness must be sensed before patterns will display
const unsigned long darknessTriggerTime=DarknessTriggerTime*1000;//*60; //conversion to milliseconds
unsigned long darknessStart; // used to store the time when darkness is first sensed

//Recovery Setup
bool recoveryMode=false; //recoveryMode is set true when the patterns have been displayed for the full time in patternDisplayTime
const unsigned long RecoveryTime=1; //15*60; //recovery time in minutes before darkness sensor is allowed to start another light show
const unsigned long recoveryTime=RecoveryTime*60*1000; //conversion to milliseconds
static unsigned long recoveryStartTime; //records when a light show ends

APA102<dataPin, clockPin> ledStrip;

#define DOORSWITCH_PIN 10 // wire doorswitch as normally open between this pin and ground
#define NEXT_PATTERN_BUTTON_PIN  9  // button between this pin and ground
#define AUTOCYCLE_SWITCH_PIN  8  // switch between this pin and ground
#define PHOTORESISTOR_PIN 12 // connect photoresistor between this pin and ground
#define BATTERYMONITOR_PIN A6 // RB2 connected between this pin and ground (A6 is digital pin 4)
#define BATTERYDISPLAY_PIN 5 // button between this pin and ground

// Create a buffer for holding colors.
#define LED_COUNT 60
rgb_color colors[LED_COUNT];
rgb_color allColors[LED_COUNT];
...
void setup()
{
 ...
 // pinMode(BATTERYMONITOR_PIN, INPUT); // this line shouldn't be necessary, try uncommenting if having problems with battery monitor
  pinMode(BATTERYDISPLAY_PIN, INPUT_PULLUP);
  ...
  for(int i = 0; i<3; i++) //Blink lights 3 times on startup to show that program is running
  {
    allOnFullBright();
    delay(400);
    allLightsOff();
  ...
  handleBatteryDisplay(0); //Display the battery charge on startup
}

void handleBatteryDisplay(bool fcnRequest)
{
  if(!fcnRequest) //invert fcnRequest since a value of 0 from digitalRead(BATTERYDISPLAY_PIN) should run the function
  {
      allLightsOff();
      delay(500);
      uint16_t batteryDisplayLEDCount = constrain((((float)analogRead(BATTERYMONITOR_PIN)/1023.0*ArefVoltage*((RB1+RB2)/RB2))-MinBatteryVoltage)/(BatteryFullChargeVoltage-MinBatteryVoltage)*(float)LED_COUNT,0,LED_COUNT); //scales the voltage reading from min battery voltage = 0 to full charge voltage (or greater) = LED_COUNT
      
      for (int i=0; i < batteryDisplayLEDCount; i++)
      {
        if(batteryDisplayLEDCount>40)
        {
            allColors[i] = (rgb_color){0,85,0}; //display green if near full charge
        }
        else if(batteryDisplayLEDCount>20)
        {
            allColors[i]= (rgb_color){85,85,0}; //display yellow if medium charge
        }
        else
        {
            allColors[i]= (rgb_color){85,0,0}; //display red if low charge
        }
      }
      
      ledStrip.write(allColors,LED_COUNT);
      delay(1000);
      allLightsOff();
  }
}

void allOnFullBright()
{
  
      for (int i = 0; i < LED_COUNT; i++)
      {
        allColors[i] = (rgb_color){85, 85, 85};
      }
      ledStrip.write(allColors,LED_COUNT);
}
...
{
  if(digitalRead(DOORSWITCH_PIN)==0)
  {
    unsigned long doorOpenedTime = millis();
    allOnFullBright(); // Turn on all LED's to full brightness (white)
  
    while((millis()-doorOpenedTime < doorOpenTimeout)&&(digitalRead(DOORSWITCH_PIN)==0))
      {
       delay(100);
      }
    allLightsOff();
    while(digitalRead(DOORSWITCH_PIN)==0){delay(100);}// do nothing if timer has elapsed and door is still open (freezes program, may act buggy)
  }
}

void handleDarknessSensor(){
  //Serial.print("handleDarknessSensor ");
  if(digitalRead(PHOTORESISTOR_PIN)==1)
  {
    //Serial.print("dark \t,");
    if(continuousDarkness!=true)
    {
      darknessStart= millis();
      continuousDarkness = true;
    }
    else
    {
      if((millis()-darknessStart)>darknessTriggerTime && !recoveryMode) //only start if not in recovery mode
      {
        displayPatterns = true; //This will start the light show when darkness has been sensed continuously for the specified time.
        patternDisplayStartTime = millis();
      }
    }
  }
  else
  {
    //Serial.print("light \t,");
    continuousDarkness=false; //light is sensed
  }
}
...
void batteryMonitor()
{
  ...
  static uint8_t batteryUnderVoltageCounter;
  if(analogRead(BATTERYMONITOR_PIN)<BatteryMinVoltCount)
  {
    //Serial.print(analogRead(BATTERYMONITOR_PIN));
    //Serial.print(",");
    batteryUnderVoltageCounter++; //increment under-voltage counter
    //Serial.print(batteryUnderVoltageCounter);
    if(batteryUnderVoltageCounter>=200)
    {
     batteryLowVoltageFault=true; //indicate fault status if 5 patterns in a row have given an error   
    }
  }
  else
  {
  batteryUnderVoltageCounter=0; //reset counter to zero if battery voltage is above the set minimum
  batteryLowVoltageFault=false; //clear the fault automatically if battery voltage is above minimum
  }
}

void startRecovery()
{
  //Serial.print("startRecovery()");
    allLightsOff();
    displayPatterns=false;
    recoveryMode=true; //recoveryMode keeps the darkness sensor from initiating a new display cycle until the next night.
...
void loop()
{
  //Serial.println("");// print each loop iteration output on a new line.
  uint8_t startTime = millis(); //records the start time of each loop iteration
  
  handleNextPatternButton();
  handleDoorSwitch();
  handleBatteryDisplay(digitalRead(BATTERYDISPLAY_PIN));

  //Serial.print("patternDisplayTime: "); //Serial.println(patternDisplayTime);
  //Serial.print("doorOpenTimeout: "); //Serial.println(doorOpenTimeout);
  //Serial.print("recoveryTime: "); //Serial.println(recoveryTime);
  //Serial.print("BatteryMinVoltCount: "); //Serial.println(BatteryMinVoltCount);
  
  if (!displayPatterns)
  {
    handleDarknessSensor();
    batteryMonitor();
    if(recoveryMode&&((millis()-recoveryStartTime)>recoveryTime))
    {
        recoveryMode=false; //end recovery if recovery time has elapsed
    }
    delay(1000); //this could be replaced with a power-down command for longer battery life.
    return; //should return to beginning of loop(), skipping any lighting display code
  }

  if(batteryLowVoltageFault)
  {
    startRecovery();
    return;
  }
  
  if((millis()-patternDisplayStartTime)>patternDisplayTime)
  {
    startRecovery();
    return;
  }
...
/*
  if(((millis()-patternDisplayStartTime)>patternDisplayTime)||batteryLowVoltageFault)
  {
      startRecovery();
      return;
  }
  */
  ...
  if (loopCount == 0)
  {
    // whenever timer resets, clear the LED colors array (all off), and check the battery voltage
    for (int i = 0; i < LED_COUNT; i++)
    {
      colors[i] = (rgb_color){0, 0, 0};      
    }
    ledStrip.write(colors,LED_COUNT);
    delay(500); // give some time for battery voltage to stabilize after turning off lights
    batteryMonitor(); // check battery at the start of each pattern
  }
...
{
  const unsigned char maxBrightness = 90;  // max brightness for the colors
  const unsigned char numCollisions = 8;  // # of collisions before pattern ends
